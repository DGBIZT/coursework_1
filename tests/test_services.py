...
def test_search_by_category(transactions, mock_json):
    result = simple_search(transactions, "Переводы")

    # Проверяем, что функция вернула данные в формате JSON
    assert result == "mocked json data"


# Тестирование поиска по описанию
def test_search_by_description(transactions, mock_json):
    result = simple_search(transactions, "Константин")

    assert result == "mocked json data"


# Тестирование отсутствия результатов
def test_no_results(transactions, mock_json):
    result = simple_search(transactions, "Несуществующая категория")

    assert result == "По данному запросу отсутствуют транзакции"


# Тестирование с пустым списком транзакций
def test_empty_transactions(mock_json):
    result = simple_search([], "Любая категория")
